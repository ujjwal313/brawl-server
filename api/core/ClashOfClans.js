...
class ClashOfClansApi {
  ...
  _tag(tag) {
    return tag
      ? `%23${tag.toUpperCase().replace(/#/g, "").replace(/O/g, "0")}`
      : "";
  }
  ...
  async warLog(tag = "", options = { limit: "", after: "", before: "" }) {
    return await this._fetchGet(
      "/clans/" + this._tag(tag) + "/warlog" + this._params(options)
    );
  }

  async cwlRounds(tag = "") {
    return await this._fetchGet(
      "/clans/" + this._tag(tag) + "/currentwar/leaguegroup"
    );
  }
  ...
  async playerVerify(tag = "", apiToken = "") {
    return await this._fetchPost(
      "/players/" + this._tag(tag) + "/verifytoken",
      { token: apiToken }
    );
  }
  ...
  async locations(
    ...
    return await this._fetchGet(
      "/locations/" + locationId + this._params(options)
    );
  }
  ...
  async clansRank(
    ...
    return await this._fetchGet(
      "/locations/" + locationId + "/rankings/clans" + this._params(options)
    );
  }
  ...
  async playersRank(
    ...
    return await this._fetchGet(
      "/locations/" + locationId + "/rankings/players" + this._params(options)
    );
  }

  async clansVersusRank(
    locationId = "",
    options = { limit: "", after: "", before: "" }
  ) {
    return await this._fetchGet(
  ...
  async warLeagues(
    ...
    return await this._fetchGet(
      "/warleagues/" + leagueId + this._params(options)
    );
  }
  ...
  async leaguesSeason(
    ...
    return await this._fetchGet(
      "/leagues/" + leagueId + "/seasons/" + seasonId + this._params(options)
    );
  }
}
