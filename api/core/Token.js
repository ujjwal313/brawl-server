...
class SupercellApi {
  ...
  async tokenGeneration() {
    try {
      ...
      if (
        !["clashofclans", "clashroyale", "brawlstars"].includes(this.platform)
      ) {
      ...
      if (
        keys &&
        keys.filter((k) => k.cidrRanges.includes(ip) && k.name === this.name)
          .length === this.limit
      ) {
        console.log("Old token in use!");
        return keys
          .filter((k) => k.cidrRanges.includes(ip) && k.name === this.name)
          .map((m) => m.key);
      } else {
        var full =
          keys.length -
        ...
        for (let i = 0; i < matchedKeys.length; i++) {
          if (
            matchedKeys[i].name === this.name &&
            !matchedKeys[i].cidrRanges.includes(ip) &&
            keysLength <= 10 &&
            count < this.limit
          ) {
            await this.revokeKey(
              keys.find((f) => f.id === matchedKeys[i].id).id
            );
        ...
        return await this.getKeys().then((keys) =>
          keys
            .filter((k) => k.cidrRanges.includes(ip) && k.name === this.name)
            .map((m) => m.key)
        );
  ...
  async createKey(ip) {
    const response = await axios.post(
      `${dev_base[this.platform]}/apikey/create`,
      {
        name: this.name,
        description: ip,
        cidrRanges: ip,
  ...
  async revokeKey(id) {
    const response = await axios.post(
      `${dev_base[this.platform]}/apikey/revoke`,
      {
        id: id,
      }
    );
