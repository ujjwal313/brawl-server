...
(async () => {
  if (process.env.COC_EMAIL && process.env.COC_PASS) {
    var token = await new Token(
      "clashofclans",
      process.env.COC_EMAIL,
      process.env.COC_PASS
    ).init();
...
app.get("*", async (request, response) => {
  ...
  if (
    request &&
    request.path &&
    request.path.match(/\/(clashofclans|clashroyale|brawlstars)\/(.*)/)
  ) {
    const query = request.path.match(
      /\/(clashofclans|clashroyale|brawlstars)\/(.*)/
    );
        ...
        const res = await axios.get(api_base[query[1]] + "/" + query[2], {
          headers: {
            ...
            Authorization: `Bearer ${
              tokens.find((t) => t.name === query[1]).token
            }`,
          },
        });
        return response
          .status(200)
          .send({ status: res.status, message: res.message, ...res.data });
      } catch (error) {
        return response
          .status(error.data.status)
          .send({ status: error.data.status, message: error.data.message });
      }
    }
  }
